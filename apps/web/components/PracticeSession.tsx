...
import { Lightbulb, Play, Loader2, ChevronDown, ChevronRight } from "lucide-react";
import { useGetForTopic, useRunCode, useSubmit, useRequestCodingHint } from "~/hooks/api/questions";
...
export function PracticeSession({ topicId }: Props) {
  ...
  // Question fetch key — increment to re-fetch a new question
  ...
  // Hooks
  const { question, isLoading, isFetching, refetch } = useGetForTopic({
    topicId,
  ...
  // Reset editor + results when question changes
  useEffect(() => {
    if (!question) return;
    setCode(question.starterCode ?? "# Write your Python solution here\n");
    setRunResults(null);
    setRunError("");
      ...
      <div className="flex flex-col items-center justify-center py-16 gap-4 text-center">
        ...
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: "var(--muted-foreground)" }}>
            You&apos;ve solved all {activeDifficulty} questions for this topic. Try a different difficulty.
          </p>
        </div>
  ...
  return (
    <div key={fetchKey}>
      ...
      {/* ── Pre-submit: editor + run/hint/submit ─────────────────────────────── */}
      {!submitResult ? (
        <>
